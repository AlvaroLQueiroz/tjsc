...
def _ler_texto_generico(driver) -> Optional[str]:
    ...
    try:
        ...
        vals = [
            (el.get_attribute("innerText") or el.text or "").strip()
            for el in ceds
            if el.is_displayed()
        ]
    ...
    # 3) editores ricos
    try:
        eds = driver.find_elements(
            By.XPATH,
            "//div[contains(@class,'cke_editable') or contains(@class,'jqte_editor') or contains(@class,'ql-editor') or contains(@class,'note-editable')]",
        )
        vals = [
            (el.get_attribute("innerText") or el.text or "").strip()
            for el in eds
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ActionChains(driver).move_to_element(btn).pause(0.05).click(
                            btn
                        ).perform()
                    except Exception:
                        continue
            ...
            try:
                modals = driver.find_elements(
                    By.XPATH,
                    "//div[contains(@class,'modal') and contains(@style,'display') and not(contains(@style,'none'))]",
                )
                if modals:
                    abriu_modal = True
            except Exception:
                ...
                try:
                    ...
                    WebDriverWait(driver, 10).until(
                        EC.presence_of_element_located(
                            (
                                By.XPATH,
                                "//div[contains(@class,'whiteboard-note')]|//fieldset[@id='fldLembretes']",
                            )
                        )
                    )
                except Exception:
